...
function SelectCreditValue({ onValueChange, defaultParam }) {
    const [data, setData] = useState([]);
    const [selectedData, setselectedData] = useState(defaultParam);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_SERVER}/creditos/lista_credito_cotizacion`);
